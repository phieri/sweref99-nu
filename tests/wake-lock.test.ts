...
type RequestWakeLock = (type: 'screen') => Promise<WakeLockSentinelLike>;
...
describe('ScreenWakeLockManager', () => {
	...
	test('should request a screen wake lock when visible', async () => {
		const request: jest.MockedFunction<RequestWakeLock> = jest.fn(
			async () => new MockWakeLockSentinel()
		);
	...
	test('should release the active wake lock', async () => {
		...
		const request: jest.MockedFunction<RequestWakeLock> = jest.fn(async () => sentinel);
		const manager = new ScreenWakeLockManager({ request }, () => false);
	...
	test('should request again after the wake lock is released externally', async () => {
		...
		const request: jest.MockedFunction<RequestWakeLock> = jest
			.fn<RequestWakeLock>()
