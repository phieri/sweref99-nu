#!/usr/bin/env python3
"""
Create a minimal proj.db containing only EPSG:4326 and EPSG:3006 definitions.
This dramatically reduces WASM size by including only Swedish coordinate systems.

Usage: python3 create_minimal_proj_db.py [output_path]
Default output: minimal_proj.db
"""
...
def create_minimal_proj_db(output_path):
    """Create minimal proj.db with only WGS84 and SWEREF99 TM"""
    
    # Create database
    conn = sqlite3.connect(output_path)
    conn.execute("PRAGMA foreign_keys = 0")
    
    # Create all required tables (simplified schema)
    schema_sql = """
-- Metadata table
...
CREATE TABLE prime_meridian(
    ...
    code TEXT NOT NULL,
    name TEXT NOT NULL,
    greenwich_longitude FLOAT NOT NULL,
    uom_auth_name TEXT NOT NULL,
    uom_code TEXT NOT NULL,
    deprecated BOOLEAN NOT NULL,
...
"""
    ...
    # Insert minimal data for WGS84 and SWEREF99 TM
    data_sql = """
-- Metadata
